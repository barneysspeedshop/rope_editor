// Adapted from Zed's sum_tree crate (Apache-2.0)
// https://github.com/zed-industries/zed

mod cursor;
mod tree_map;

// Re-export everything from the inline sum_tree module
...
/// Drain all elements from a heapless Vec (heapless has no drain method).
fn arrayvec_drain_all<U: Clone, const N: usize>(vec: &mut ArrayVec<U, N>) -> std::vec::Vec<U> {
    let result: std::vec::Vec<U> = vec.iter().cloned().collect();
    vec.clear();
    result
}

/// Drain the first `count` elements from a heapless Vec.
fn arrayvec_drain_front<U: Clone, const N: usize>(
    vec: &mut ArrayVec<U, N>,
    count: usize,
) -> std::vec::Vec<U> {
    let taken: std::vec::Vec<U> = vec.iter().take(count).cloned().collect();
    let remaining: ArrayVec<U, N> = vec.iter().skip(count).cloned().collect();
    *vec = remaining;
    taken
}
            ...
            if let Some(tree) = res {
                if child_trees.len() < 2 * TREE_BASE {
                    ...
                    let new_child_summaries = {
                        ...
                        res.extend(arrayvec_drain_front(child_summaries, TREE_BASE).into_iter());
                        res
                    };
                    let tree = SumTree(Arc::new(Node::Internal {
                        ...
                        child_trees: {
                            ...
                            res.extend(arrayvec_drain_front(child_trees, TREE_BASE).into_iter());
                            res
                        },
                    }));
        ...
        match (small.0.as_ref(), Arc::make_mut(&mut large.0)) {
            (
                ...
                if total_child_count <= 2 * TREE_BASE {
                    ...
                    all_trees.extend(arrayvec_drain_all(child_trees).into_iter());
                    *child_trees = all_trees;

                    let mut all_summaries = small_child_summaries.clone();
                    all_summaries.extend(arrayvec_drain_all(child_summaries).into_iter());
            ...
            (
                ...
                if total_child_count <= 2 * TREE_BASE {
                    ...
                    all_items.extend(arrayvec_drain_all(items).into_iter());
                    *items = all_items;

                    let mut all_summaries = small_item_summaries.clone();
                    all_summaries.extend(arrayvec_drain_all(item_summaries).into_iter());
