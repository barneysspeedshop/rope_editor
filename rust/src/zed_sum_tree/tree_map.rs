...
// https://github.com/zed-industries/zed/blob/main/crates/sum_tree/src/tree_map.rs
...
impl<K: Clone + Ord, V: Clone> TreeMap<K, V> {
    ...
    pub fn get(&self, key: &K) -> Option<&V> {
        let (.., item) = self
            .0
            .find::<MapKeyRef<'_, K>, _>((), &MapSeekTargetAdaptor(key), Bias::Left);
    ...
    pub fn remove(&mut self, key: &K) -> Option<V> {
        ...
        if SeekTarget::cmp(&key, &cursor.end(), ()) == Ordering::Equal {
    ...
    pub fn closest<'a>(&'a self, key: &K) -> Option<(&'a K, &'a V)> {
        let mut cursor = self.0.cursor::<MapKeyRef<'a, K>>(());
        cursor.seek(&MapSeekTargetAdaptor(key), Bias::Right);
        cursor.prev();
    ...
    pub fn iter_from<'a>(&'a self, from: &'a K) -> impl Iterator<Item = (&'a K, &'a V)> + 'a {
        let mut cursor = self.0.cursor::<MapKeyRef<'a, K>>(());
        cursor.seek(&MapSeekTargetAdaptor(from), Bias::Left);
...
impl<K: Ord> PartialOrd for MapKeyRef<'_, K> {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl<K: Ord> Ord for MapKeyRef<'_, K> {
...
impl<K: Ord> PartialEq for MapKeyRef<'_, K> {
    fn eq(&self, other: &Self) -> bool {
        self.cmp(other) == std::cmp::Ordering::Equal
    }
}

impl<K: Ord> Eq for MapKeyRef<'_, K> {}
